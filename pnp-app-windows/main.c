...
// COM Port number for SensorTile.Box
static int comPort;

// TODO: Fill in DIGITALTWIN_DEVICE_CAPABILITY_MODEL_INLINE_DATA if want to make device self-describing.
#define DIGITALTWIN_DEVICE_CAPABILITY_MODEL_INLINE_DATA "{}"

static const char *digitalTwinSample_CustomProvisioningData = "{"
                                                              "\"__iot:interfaces\":"
                                                              "{"
                                                              "\"CapabilityModelId\": \"urn:iotPnpPublicPreviewStSensor:SensorTileBox_pnp:1\" ,"
...
static void setup()
{
    ...
    if (registerDevice(false))
    {
        ...
        if (pnp_device_initialize(buff, certificates, comPort) == 0)
        {
...
int main(int argc, char *argv[])
{
    ...
    {
        LogError("\r\nUSAGE: pnp-app-windows [Device ID] [DPS ID Scope] [DPS symmetric key] [COM Port Number]");
        return 1;
    }
